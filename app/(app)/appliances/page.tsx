...
import { deleteAppliance, updateAppliance } from "@/features/appliances/actions";
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
      <section className="space-y-4">
        <div className="rounded-lg bg-[#ffd36a] p-5 text-[#514227] shadow-sm">
          <h1 className="text-2xl font-semibold">เครื่องใช้ไฟฟ้า</h1>
          <p className="mt-1 text-sm text-[#705b2f]">ติดตามเครื่องใช้และอุปกรณ์สำคัญในบ้าน</p>
        </div>
        <form
          action="/appliances"
        ...
        <div className="grid gap-3">
          {appliances.length ? (
            appliances.map((item) => (
              <Card key={item.id} className="border-0 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    {[item.brand, item.model, item.warranty_end_date ? `ประกันถึง ${formatDate(item.warranty_end_date)}` : null]
                      .filter(Boolean)
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle className="text-base">ยังไม่มีเครื่องใช้ไฟฟ้า</CardTitle>
                <CardDescription>เพิ่มเครื่องใช้ไฟฟ้ารายการแรก</CardDescription>
              </CardHeader>
            </Card>
          )}
        </div>
      ...
      <Card className="border-0 bg-white shadow-sm">
        ...
        <CardContent>
          {home ? (
            ...
            <p className="text-sm text-muted-foreground">สร้างบ้านก่อนเพิ่มเครื่องใช้ไฟฟ้า</p>
          )}
        </CardContent>
      </Card>
    </div>
